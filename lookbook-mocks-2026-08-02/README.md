...
- Interactivity is visual-state only (agreed 2026-07-28): no data capture, no localStorage, no network. External links open new tabs. Step navigation is **fully self-serve** (see below): only the step 1→2 transition can ever be data-driven (Navvy sees screener completion); after that the app has no signal, so every step change is a student's own click. In these mocks the controls link between the four state files as a stand-in for the page swap.
...
- **Step navigation (2026-08-02, both shells, fully self-serve):** every step is reachable at any time — students can skip ahead or go back whenever they want, because after the screener Navvy is blind (no signal on applications, follow-ups, or decisions; only the step 1→2 transition can be auto-advanced on screener completion). Done bars carry a green "← Go back" link (desktop: right end of the bar; mobile: right of the DONE label). The current card carries a coral-outlined self-report button below its content — 01 "I've already checked my eligibility — show step 2" · 02 "I've submitted my application — show step 3" · 03 "I've finished my follow-ups — show step 4"; the final step has none. Upcoming steps carry a coral "I'm on this step →" jump link (desktop: right-aligned in the card header; mobile: stacked under the title) — never a lock or gate. A footnote under the list reassures students that updating a step never changes the real application. In the mocks these controls link between the state files; in the app they're front-end page/state swaps only (no data capture).
